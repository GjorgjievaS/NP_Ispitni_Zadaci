...
class LogSystem<T extends ILog & Comparable<T>> {
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = logs1 -> logs1.stream()
                .filter(l -> l.getType().equals("INFO"))
                .collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<T> secondLogProcessor = logs1 -> logs1.stream()
                .filter(l->l.getMessage().length()<100)
                .collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<T> thirdLogProcessor = logs1 -> logs1.stream().sorted().collect(Collectors.toCollection(ArrayList::new));
